...
osSemaphoreId myCountingHandle;
...
void MX_FREERTOS_Init(void) {
  ...
  //myCountingHandle = osSemaphoreCreate(osSemaphore(myCounting), 2);

  /* USER CODE BEGIN RTOS_SEMAPHORES */
  /* add semaphores, ... */
	myCountingHandle = xSemaphoreCreateCounting(3, 0);
...
void sendQueue(void const * argument)
{
  ...
  for(;;)
  {
		if (HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_0) == GPIO_PIN_RESET)
		{
			...
			if (HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_0) == GPIO_PIN_RESET)
			{
				status = xSemaphoreGive(myCountingHandle);
				if(status == pdPASS) {
					printf("send success\r\n");
				} else {
					printf("send failure\r\n");
...
void receiveQueue(void const * argument)
{
  ...
  for(;;)
  {
		if (HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_1) == GPIO_PIN_RESET)
		{
			...
			if (HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_1) == GPIO_PIN_RESET)
			{
				status = xSemaphoreTake(myCountingHandle, 0);
				if(status == pdTRUE) {
